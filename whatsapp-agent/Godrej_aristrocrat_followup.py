...
if __name__ == '__main__':
    ...
    log_with_timestamp("Step 1: Removing internal duplicates from AnarockGodrejList worksheet...")
    sheet = get_google_sheet()
    if sheet:
        remove_duplicate_entries(sheet, worksheet_name, phone_column, last_message_datetime_col, testing=False)
        log_with_timestamp("Internal duplicate removal completed.")
        
        # Step 2: Remove cross-sheet duplicates (remove from AnarockGodrejList if exists in IPC Outreach)
        log_with_timestamp("Step 2: Removing cross-sheet duplicates (AnarockGodrejList numbers that exist in IPC Outreach)...")
        remove_cross_sheet_duplicates(sheet, 'IPC Outreach', worksheet_name, phone_column, testing=False)
        log_with_timestamp("Cross-sheet duplicate removal completed.")
    else:
